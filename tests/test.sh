...
# Git for Windows does not ship util-linux flock; Linux CI uses the real binary.
...
if is_server_name "" || is_server_name "   " || is_server_name $'bad\tname' \
    || is_server_name " 前导空格" || is_server_name "尾随空格 " \
    || is_server_name "$long_server_name"; then
    printf 'FAIL: invalid server name accepted\n' >&2
    exit 1
fi
is_day_key "2026-02-28" || { printf 'FAIL: valid day key rejected\n' >&2; exit 1; }
if is_day_key "2026-02-30" || is_day_key "2026-13-01"; then
    printf 'FAIL: invalid day key accepted\n' >&2
    exit 1
fi
...
for removed_function in run_command_poll control_api_call register_control_instance; do
    if declare -F "$removed_function" >/dev/null; then
        printf 'FAIL: removed remote-control function still exists: %s\n' "$removed_function" >&2
        exit 1
    fi
done
grep -Fq '无需部署 Cloudflare Worker' "${ROOT_DIR}/README.md"
if grep -Eq '远程控制|/reboot|/shutdown|/servers|/remove' "${ROOT_DIR}/README.md"; then
    printf 'FAIL: README still advertises remote control\n' >&2
    exit 1
fi
...
printf '%s\n' '{"ok":true,"result":{}}' > "${TEST_DIR}/telegram-invalid.json"
if telegram_updates_cursor "${TEST_DIR}/telegram-invalid.json" >/dev/null 2>&1; then
    printf 'FAIL: malformed Telegram update response was accepted\n' >&2
    exit 1
fi
...
test_setup_api_config_escaping() (
    WORK_DIR="${TEST_DIR}/setup-api-escaping"
    mkdir -p "$WORK_DIR"
    local response="${WORK_DIR}/response.json"
    curl() {
        cp -- "$2" "${WORK_DIR}/captured.conf"
        printf '%s\n' '{"ok":true}' > "$response"
    }
    setup_api_call '123456:abcdefghijklmnopqrstuvwxyzABCDE_12345' sendMessage "$response" \
        'text=name "quoted" \backslash'
    grep -Fqx 'data-urlencode = "text=name \"quoted\" \\backslash"' "${WORK_DIR}/captured.conf"
)
(test_setup_api_config_escaping)
...
grep -Fq -- '--property=Slice=vps-monitor-login.slice' "${TEST_DIR}/login-alert-hook"
...
test_status() {
    ...
    local SYSTEMD_DIR="${DATA_DIR}/systemd"
    mkdir -p "$DATA_DIR" "$SYSTEMD_DIR"
    printf '#!/usr/bin/env sh\n# --property=Slice=vps-monitor-login.slice\nexit 0\n' > "$LOGIN_HOOK_PATH"; chmod 0700 "$LOGIN_HOOK_PATH"
    : > "${SYSTEMD_DIR}/vps-monitor-login.slice"
    ...
    rm -f -- "${SYSTEMD_DIR}/vps-monitor-login.slice"
    status_output="$(run_status)"
    grep -Fqx '登录提醒: 异常' <<< "$status_output"
    : > "${SYSTEMD_DIR}/vps-monitor-login.slice"
...
assert_equal "" "$STATE_DAY" "legacy state starts without daily key"
assert_equal "0" "$DAY_RX" "legacy state starts with zero daily rx"
...
STATE_DAY="2026-08-04"
DAY_RX=100; DAY_TX=200; DAY_CPU_BUSY=10; DAY_CPU_TOTAL=50; DAY_SECONDS=60
LAST_TS="$(date -d '2026-08-04 23:55:00 UTC' +%s)"
CURRENT_TS="$(date -d '2026-08-05 00:05:00 UTC' +%s)"
add_day_delta 2026-08-05 600 6000 8000 300 600
read -r archived_rx archived_tx archived_busy archived_total archived_seconds < "${DATA_DIR}/day-2026-08-04.tsv"
assert_equal "3100" "$archived_rx" "previous day rx split"
assert_equal "4200" "$archived_tx" "previous day tx split"
assert_equal "160" "$archived_busy" "previous day cpu split"
assert_equal "350" "$archived_total" "previous day cpu total split"
assert_equal "360" "$archived_seconds" "previous day duration split"
assert_equal "2026-08-05" "$STATE_DAY" "new day selected"
assert_equal "3000" "$DAY_RX" "new day rx split"
assert_equal "4000" "$DAY_TX" "new day tx split"
assert_equal "150" "$DAY_CPU_BUSY" "new day cpu split"
assert_equal "300" "$DAY_CPU_TOTAL" "new day cpu total split"
assert_equal "300" "$DAY_SECONDS" "new day duration split"

STATE_DAY="2026-08-01"
DAY_RX=0; DAY_TX=0; DAY_CPU_BUSY=0; DAY_CPU_TOTAL=0; DAY_SECONDS=0
LAST_TS="$(date -d '2026-08-01 12:00:00 UTC' +%s)"
CURRENT_TS="$(date -d '2026-08-03 12:00:00 UTC' +%s)"
multi_day_elapsed=$((CURRENT_TS - LAST_TS))
add_day_delta 2026-08-03 "$multi_day_elapsed" "$multi_day_elapsed" "$((multi_day_elapsed * 2))" \
    "$multi_day_elapsed" "$((multi_day_elapsed * 2))"
read -r archived_rx archived_tx archived_busy archived_total archived_seconds < "${DATA_DIR}/day-2026-08-02.tsv"
assert_equal "86400" "$archived_rx" "multi-day full day rx"
assert_equal "172800" "$archived_tx" "multi-day full day tx"
assert_equal "86400" "$archived_seconds" "multi-day full day duration"
assert_equal "2026-08-03" "$STATE_DAY" "multi-day current day"
assert_equal "43200" "$DAY_RX" "multi-day current rx"
assert_equal "43200" "$DAY_SECONDS" "multi-day current duration"

(
    export TZ=America/New_York
    DATA_DIR="${TEST_DIR}/dst-day"
    mkdir -p "$DATA_DIR"
    STATE_DAY="2026-03-08"
    DAY_RX=0; DAY_TX=0; DAY_CPU_BUSY=0; DAY_CPU_TOTAL=0; DAY_SECONDS=0
    LAST_TS="$(date -d '2026-03-08 00:00:00' +%s)"
    CURRENT_TS="$(date -d '2026-03-09 00:00:00' +%s)"
    dst_elapsed=$((CURRENT_TS - LAST_TS))
    add_day_delta 2026-03-09 "$dst_elapsed" "$dst_elapsed" "$dst_elapsed" "$dst_elapsed" "$dst_elapsed"
    read -r _ _ _ _ archived_seconds < "${DATA_DIR}/day-2026-03-08.tsv"
    assert_equal "$dst_elapsed" "$archived_seconds" "daily archive ends at natural midnight"
    if [[ "$(date -d '2026-03-08 12:00:00' +%z)" == -0400 ]]; then
        assert_equal "82800" "$archived_seconds" "DST day keeps its natural 23-hour length"
    fi
)
...
test_daily_delivery() (
    DATA_DIR="${TEST_DIR}/daily-delivery"
    STATE_FILE="${DATA_DIR}/state.tsv"
    mkdir -p "$DATA_DIR"
    yesterday="$(date -d yesterday +%F)"
    older_day="$(date -d '2 days ago' +%F)"
    printf '864000000000\t432000000000\t29\t100\t86400\n' > "${DATA_DIR}/day-${yesterday}.tsv"
    printf '1000000000\t2000000000\t1\t2\t3600\n' > "${DATA_DIR}/day-${older_day}.tsv"
    reset_state_defaults
    # shellcheck disable=SC2317,SC2329 # Called by the sourced daily command.
    load_config() { :; }
    # shellcheck disable=SC2317,SC2329 # Called by the sourced daily command.
    acquire_lock() { :; }
    # shellcheck disable=SC2317,SC2329 # Daily archive is prepared directly for this delivery test.
    collect_locked() { :; }
    # shellcheck disable=SC2317,SC2329 # Captures daily Telegram delivery.
    send_message() {
        printf '%s' "$1" > "${DATA_DIR}/captured-message"
        printf 'sent\n' >> "${DATA_DIR}/send-count"
    }

    assert_equal "report: sent" "$(run_report)" "daily report delivery"
    [[ ! -e "${DATA_DIR}/day-${yesterday}.tsv" && ! -e "${DATA_DIR}/sent-day-${yesterday}" \
        && ! -e "${DATA_DIR}/day-${older_day}.tsv" && ! -e "${DATA_DIR}/sent-day-${older_day}" ]] \
        || { printf 'FAIL: delivered daily archive was not cleaned up\n' >&2; exit 1; }
    grep -Fqx "测试服务器 ${yesterday} 日报" "${DATA_DIR}/captured-message"
    assert_equal "report: nothing-pending" "$(run_report)" "daily duplicate prevention"
    assert_equal "2" "$(wc -l < "${DATA_DIR}/send-count")" "daily backlog send count"
)
(test_daily_delivery)

test_daily_failure_retry() (
    DATA_DIR="${TEST_DIR}/daily-failure"
    STATE_FILE="${DATA_DIR}/state.tsv"
    mkdir -p "$DATA_DIR"
    yesterday="$(date -d yesterday +%F)"
    archive="${DATA_DIR}/day-${yesterday}.tsv"
    printf '1\t2\t1\t2\t60\n' > "$archive"
    reset_state_defaults
    load_config() { :; }
    acquire_lock() { :; }
    collect_locked() { :; }
    send_message() { return 1; }
    if run_report >/dev/null 2>&1; then
        printf 'FAIL: failed daily delivery was reported as success\n' >&2
        exit 1
    fi
    [[ -e "$archive" ]] || { printf 'FAIL: failed daily delivery discarded its archive\n' >&2; exit 1; }
)
(test_daily_failure_retry)
...
grep -Fxq 'OnCalendar=*-*-* 12:00:00' "${SYSTEMD_DIR}/vps-monitor-report.timer"
...
grep -Fxq 'MemoryMax=128M' "${SYSTEMD_DIR}/vps-monitor-collect.service"
grep -Fxq 'TasksMax=32' "${SYSTEMD_DIR}/vps-monitor-collect.service"
grep -Fxq 'OnCalendar=Mon *-*-* 12:10:00' "${SYSTEMD_DIR}/vps-monitor-weekly.timer"
...
for bounded_service in report weekly monthly boot; do
    grep -Fxq 'MemoryMax=128M' "${SYSTEMD_DIR}/vps-monitor-${bounded_service}.service"
    grep -Fxq 'TasksMax=32' "${SYSTEMD_DIR}/vps-monitor-${bounded_service}.service"
done
...
grep -Fxq 'MemoryMax=256M' "${SYSTEMD_DIR}/vps-monitor-auto-update.service"
grep -Fxq 'TasksMax=64' "${SYSTEMD_DIR}/vps-monitor-auto-update.service"
...
grep -Fxq 'MemoryMax=128M' "${SYSTEMD_DIR}/vps-monitor-login.slice"
grep -Fxq 'TasksMax=64' "${SYSTEMD_DIR}/vps-monitor-login.slice"
[[ ! -e "${SYSTEMD_DIR}/vps-monitor-command.service" \
    && ! -e "${SYSTEMD_DIR}/vps-monitor-command.timer" ]] \
    || { printf 'FAIL: remote control systemd units were generated\n' >&2; exit 1; }

test_remove_legacy_components() (
    local SYSTEMD_DIR="${TEST_DIR}/legacy-control/systemd"
    local CONFIG_DIR="${TEST_DIR}/legacy-control/config"
    local DATA_DIR="${TEST_DIR}/legacy-control/data"
    mkdir -p "$SYSTEMD_DIR" "$CONFIG_DIR" "$DATA_DIR"
    : > "${SYSTEMD_DIR}/vps-monitor-command.service"
    : > "${SYSTEMD_DIR}/vps-monitor-command.timer"
    : > "${CONFIG_DIR}/command_offset"
    : > "${CONFIG_DIR}/instance_id"
    : > "${CONFIG_DIR}/control_url"
    ...
    for legacy_path in \
        ...
        "${CONFIG_DIR}/instance_id" \
        "${CONFIG_DIR}/control_url" \
        "${DATA_DIR}/samples.tsv"; do
        [[ ! -e "$legacy_path" ]] \
            || { printf 'FAIL: legacy remote-control path was not removed: %s\n' "$legacy_path" >&2; exit 1; }
    done
)
(test_remove_legacy_components)

if command -v systemd-analyze >/dev/null 2>&1; then
    systemd-analyze verify "${SYSTEMD_DIR}"/*.service "${SYSTEMD_DIR}"/*.timer "${SYSTEMD_DIR}"/*.slice
fi
...
(
    ...
    APPLY_UPDATE_LOGIN_SLICE_EXISTED=0
    ...
    for unit_name in vps-monitor-weekly.{service,timer} vps-monitor-boot.service \
        vps-monitor-auto-update.{service,timer} vps-monitor-login.slice; do
...
(
    ...
    [[ ! -e "${SYSTEMD_DIR}/vps-monitor-command.service" \
        && ! -e "${SYSTEMD_DIR}/vps-monitor-command.timer" \
        && ! -e "${SYSTEMD_DIR}/vps-monitor-login.slice" ]] \
...
test_bin_link_conflict_return() (
    BIN_PATH="${TEST_DIR}/bin-link-conflict/vps-monitor"
    SCRIPT_PATH="${TEST_DIR}/bin-link-conflict/installed.sh"
    mkdir -p "$(dirname -- "$BIN_PATH")"
    : > "$BIN_PATH"
    : > "$SCRIPT_PATH"
    if ensure_bin_link return; then
        printf 'FAIL: nonfatal command-link guard accepted an occupied path\n' >&2
        exit 1
    fi
    [[ -f "$BIN_PATH" && ! -L "$BIN_PATH" ]] \
        || { printf 'FAIL: command-link guard modified an unrelated file\n' >&2; exit 1; }
)
(test_bin_link_conflict_return)
...
(
    ...
    for unit_name in vps-monitor-weekly.{service,timer} vps-monitor-boot.service \
        vps-monitor-auto-update.{service,timer} vps-monitor-command.{service,timer} \
        vps-monitor-login.slice; do
        [[ ! -e "${SYSTEMD_DIR}/${unit_name}" ]] \
...
(
    ...
    remove_legacy_components() { : > "${DOCTOR_ROOT}/legacy-control-removed"; }
    install_systemd_units() {
    ...
    [[ -e "${DOCTOR_ROOT}/units-repaired" && -e "${DOCTOR_ROOT}/pam-repaired" \
        ...
        && -e "${DOCTOR_ROOT}/legacy-control-removed" ]] \
