...
is_day_key() {
    ...
    [[ "$value" =~ ^[0-9]{4}-(0[1-9]|1[0-2])-([0-2][0-9]|3[01])$ ]] || return 1
    normalized="$(date -d "${value} 00:00:00" +%F 2>/dev/null)" || return 1
    [[ "$normalized" == "$value" ]]
}
...
cleanup_failed_apply_units() {
    ...
    if (( APPLY_UPDATE_LOGIN_SLICE_EXISTED == 0 )); then
        systemctl stop vps-monitor-login.slice >/dev/null 2>&1 || true
        rm -f -- "${SYSTEMD_DIR}/vps-monitor-login.slice" || true
    fi
...
cleanup_failed_install() {
    ...
    systemctl stop 'vps-monitor-login-alert-*.service' vps-monitor-login.slice >/dev/null 2>&1 || true
    ...
    rm -f -- "${SYSTEMD_DIR}"/vps-monitor-{collect,report,weekly,monthly,auto-update,command}.{service,timer} \
        "${SYSTEMD_DIR}/vps-monitor-boot.service" "${SYSTEMD_DIR}/vps-monitor-login.slice" \
        "$BIN_PATH" "$UPDATE_LOCK_FILE" || true
...
reset_state_defaults() {
    ...
    STATE_DAY=""; DAY_RX=0; DAY_TX=0; DAY_CPU_BUSY=0; DAY_CPU_TOTAL=0; DAY_SECONDS=0
...
load_state() {
    ...
    while IFS=$'\t' read -r key value || [[ -n "${key:-}" ]]; do
        case "$key" in
            ...
            day) is_day_key "$value" && STATE_DAY="$value" ;;
            day_rx) is_uint "$value" && DAY_RX="$value" ;;
            day_tx) is_uint "$value" && DAY_TX="$value" ;;
            day_cpu_busy) is_uint "$value" && DAY_CPU_BUSY="$value" ;;
            day_cpu_total) is_uint "$value" && DAY_CPU_TOTAL="$value" ;;
            day_seconds) is_uint "$value" && DAY_SECONDS="$value" ;;
...
save_state() {
    ...
    printf '%s\t%s\n' \
        ...
        day_rx "$DAY_RX" \
        day_tx "$DAY_TX" \
        day_cpu_busy "$DAY_CPU_BUSY" \
        day_cpu_total "$DAY_CPU_TOTAL" \
...
archive_current_day() {
    is_day_key "$STATE_DAY" || return 0
    (( DAY_SECONDS > 0 )) || return 0
    local archive="${DATA_DIR}/day-${STATE_DAY}.tsv" temporary
    temporary="$(mktemp "${DATA_DIR}/day.XXXXXXXX")"
    printf '%s\t%s\t%s\t%s\t%s\n' \
        "$DAY_RX" "$DAY_TX" "$DAY_CPU_BUSY" "$DAY_CPU_TOTAL" "$DAY_SECONDS" > "$temporary"
    chmod 0600 "$temporary"
    mv -f -- "$temporary" "$archive"
}

roll_day_without_delta() {
    local current_day="$1"
    is_day_key "$current_day" || fatal "无法计算当前统计日期。"
    if [[ -n "$STATE_DAY" && "$STATE_DAY" != "$current_day" ]]; then
        archive_current_day
        STATE_DAY="$current_day"
        DAY_RX=0; DAY_TX=0; DAY_CPU_BUSY=0; DAY_CPU_TOTAL=0; DAY_SECONDS=0
    elif [[ -z "$STATE_DAY" ]]; then
        STATE_DAY="$current_day"
    fi
}
...
add_day_delta() {
    local current_day="$1" elapsed="$2" rx_delta="$3" tx_delta="$4" busy_delta="$5" total_delta="$6"
    if [[ -z "$STATE_DAY" ]]; then
        STATE_DAY="$(date -d "@${LAST_TS}" +%F)" || fatal "无法初始化每日统计日期。"
        is_day_key "$STATE_DAY" || fatal "每日统计初始日期异常。"
    fi
    if [[ "$STATE_DAY" == "$current_day" ]]; then
        DAY_RX=$((DAY_RX + rx_delta)); DAY_TX=$((DAY_TX + tx_delta))
        DAY_CPU_BUSY=$((DAY_CPU_BUSY + busy_delta)); DAY_CPU_TOTAL=$((DAY_CPU_TOTAL + total_delta))
        DAY_SECONDS=$((DAY_SECONDS + elapsed))
        return
    fi

    local cursor="$LAST_TS" segment_day boundary segment_end segment_seconds cumulative_seconds
    local cumulative_rx cumulative_tx cumulative_busy cumulative_total
    local allocated_rx=0 allocated_tx=0 allocated_busy=0 allocated_total=0
    local segment_rx segment_tx segment_busy segment_total
    while (( cursor < CURRENT_TS )); do
        segment_day="$(date -d "@${cursor}" +%F)" || fatal "无法计算跨日统计日期。"
        is_day_key "$segment_day" || fatal "跨日统计日期异常。"
        boundary="$(date -d "${segment_day} + 1 day 00:00:00" +%s)" \
            || fatal "无法计算跨日统计边界。"
        if ! is_uint "$boundary" || (( boundary <= cursor )); then
            fatal "跨日统计边界异常。"
        fi
        ...
        if [[ -n "$STATE_DAY" && "$STATE_DAY" != "$segment_day" ]]; then
            archive_current_day
            DAY_RX=0; DAY_TX=0; DAY_CPU_BUSY=0; DAY_CPU_TOTAL=0; DAY_SECONDS=0
        fi
        STATE_DAY="$segment_day"
        DAY_RX=$((DAY_RX + segment_rx)); DAY_TX=$((DAY_TX + segment_tx))
        DAY_CPU_BUSY=$((DAY_CPU_BUSY + segment_busy)); DAY_CPU_TOTAL=$((DAY_CPU_TOTAL + segment_total))
        DAY_SECONDS=$((DAY_SECONDS + segment_seconds))
...
collect_locked() {
    load_state
    read_counters
    local current_month current_week current_day elapsed rx_delta tx_delta busy_delta total_delta
    current_month="$(date -d "@${CURRENT_TS}" +%Y-%m)"
    current_week="$(date -d "@${CURRENT_TS}" +%G-W%V)"
    current_day="$(date -d "@${CURRENT_TS}" +%F)"
    ...
    if (( LAST_TS == 0 )) || [[ "$LAST_BOOT" != "$CURRENT_BOOT" || "$LAST_INTERFACE" != "$INTERFACE" ]]; then
        ...
        roll_day_without_delta "$current_day"
    ...
    add_day_delta "$current_day" "$elapsed" "$rx_delta" "$tx_delta" "$busy_delta" "$total_delta"
    LAST_TS="$CURRENT_TS"; LAST_BOOT="$CURRENT_BOOT"; LAST_INTERFACE="$INTERFACE"
    LAST_RX="$CURRENT_RX"; LAST_TX="$CURRENT_TX"
    LAST_CPU_BUSY="$CURRENT_CPU_BUSY"; LAST_CPU_TOTAL="$CURRENT_CPU_TOTAL"
    save_state
...
send_message() {
    local message="$1" failure_mode="${2:-fatal}" request_base="${WORK_DIR:-$DATA_DIR}"
    ...
    if (( result != 0 )); then
        ...
        fatal "Telegram 消息发送失败；Token、UID 和消息内容均未写入日志。"
    fi
}
...
cleanup_sent_days() {
    local marker day archive
    for marker in "$DATA_DIR"/sent-day-????-??-??; do
        [[ -f "$marker" ]] || continue
        day="${marker##*/sent-day-}"
        is_day_key "$day" || continue
        archive="${DATA_DIR}/day-${day}.tsv"
        rm -f -- "$archive" "$marker"
    done
}

oldest_unsent_day() {
    local current_day candidate day oldest=""
    current_day="$(date +%F)"
    for candidate in "$DATA_DIR"/day-????-??-??.tsv; do
        [[ -f "$candidate" ]] || continue
        day="${candidate##*/day-}"; day="${day%.tsv}"
        is_day_key "$day" || continue
        [[ "$day" < "$current_day" ]] || continue
        [[ ! -e "${DATA_DIR}/sent-day-${day}" ]] || continue
        if [[ -z "$oldest" || "$day" < "$oldest" ]]; then
            oldest="$day"
        fi
    done
    [[ -n "$oldest" ]] || return 1
...
run_report() {
    load_config
    acquire_lock || fatal "统计任务繁忙，本次日报将由 systemd 自动重试。"
    collect_locked >/dev/null
    cleanup_sent_days
    local report_day archive marker rx tx busy total seconds message sent_count=0
    while report_day="$(oldest_unsent_day)"; do
        archive="${DATA_DIR}/day-${report_day}.tsv"; marker="${DATA_DIR}/sent-day-${report_day}"
        read -r rx tx busy total seconds < "$archive"
        if ! is_uint "$rx" || ! is_uint "$tx" || ! is_uint "$busy" || ! is_uint "$total" \
            || ! is_uint "$seconds" || (( seconds == 0 || busy > total )); then
            fatal "每日统计文件格式异常。"
        fi
        message="$(format_day_message "$report_day" "$rx" "$tx" "$busy" "$total" "$seconds")"
        send_message "$message" || return 1
        : > "$marker"; chmod 0600 "$marker"
        cleanup_sent_days
        LAST_REPORT="$(date +%s)"; save_state
        sent_count=$((sent_count + 1))
    done
    if (( sent_count == 0 )); then
        printf 'report: nothing-pending\n'
        return
    fi
    printf 'report: sent\n'
}
...
run_status() {
    ...
    local last_sample="暂无" last_report="暂无" state_size=0 now
    local login_alert="异常" boot_alert="正常" auto_update="正常"
    local timers="正常" timer service service_result
    local month_total week_total current_month current_week current_day candidate key
    local pending_days=0 pending_months=0 pending_weeks=0 timezone
    now="$(date +%s)"
    current_month="$(date +%Y-%m)"; current_week="$(date +%G-W%V)"; current_day="$(date +%F)"; timezone="$(date +%Z)"
    ...
    if (( LAST_TS == 0 || LAST_TS > now + 300 || now - LAST_TS > MAX_COLLECTION_AGE_SECONDS )); then
        timers="异常"
    fi
    monitored_interface_is_readable || timers="异常"
    if [[ -x "$LOGIN_HOOK_PATH" && -r "$PAM_SSHD_FILE" \
        && -f "${SYSTEMD_DIR}/vps-monitor-login.slice" ]] \
        && grep -Fq -- '--property=Slice=vps-monitor-login.slice' "$LOGIN_HOOK_PATH" \
        && grep -Fqx "$(pam_login_line)" "$PAM_SSHD_FILE" && sshd_pam_is_enabled \
        && ! systemctl is-failed --quiet vps-monitor-login.slice; then
        login_alert="正常"
    fi
    month_total=$((MONTH_RX + MONTH_TX)); week_total=$((WEEK_RX + WEEK_TX))
    for candidate in "$DATA_DIR"/day-????-??-??.tsv; do
        [[ -f "$candidate" ]] || continue
        key="${candidate##*/day-}"; key="${key%.tsv}"
        is_day_key "$key" || continue
        [[ "$key" < "$current_day" && ! -e "${DATA_DIR}/sent-day-${key}" ]] && pending_days=$((pending_days + 1))
    done
    for candidate in "$DATA_DIR"/month-????-??.tsv; do
        [[ -f "$candidate" ]] || continue
    ...
    printf '程序版本: %s\n服务器名称: %s\n监控网卡: %s\n本地时区: %s\n定时任务: %s\n自动更新: %s\n登录提醒: %s\n开机提醒: %s\n最近采样: %s\n最近日报: %s\n本周流量: %.2f TB\n本月流量: %.2f TB\n待发日报: %s 条\n待发周报: %s 条\n待发月报: %s 条\n状态文件: %s 字节\n' \
        ...
        "$pending_days" "$pending_weeks" "$pending_months" "$state_size"
}

setup_api_call() {
    local token="$1" method="$2" response="$3"; shift 3
    local config="${WORK_DIR}/setup-api.conf" item escaped
    printf '%s\n' \
...
for update in updates:
    ...
    if (message.get("text") == expected and chat.get("type") == "private"
            and sender.get("is_bot") is False and isinstance(sender_id, int)
        ...
        if setup_api_call "$token" getUpdates "$response" "timeout=10" "limit=100" "offset=${cursor}"; then
            ...
            if is_telegram_uid "$uid"; then
                ...
                setup_api_call "$token" sendMessage "$response" \
                    "chat_id=${uid}" "text=VPS Monitor：${SERVER_NAME} 通知绑定成功。" \
...
install_systemd_units() {
    ...
    cat > "${SYSTEMD_DIR}/vps-monitor-login.slice" <<EOF
[Unit]
Description=Aggregate resource limits for VPS SSH login alerts
[Slice]
MemoryMax=128M
TasksMax=64
EOF
...
[Unit]
Description=Send previous calendar day VPS Telegram report
...
[Unit]
Description=Send yesterday's VPS report daily at noon
[Timer]
OnCalendar=*-*-* 12:00:00
Persistent=true
AccuracySec=1min
...
EOF
    ...
    chmod 0644 "${SYSTEMD_DIR}/vps-monitor-boot.service" "${SYSTEMD_DIR}/vps-monitor-login.slice"
}
...
remove_legacy_components() {
    ...
    rm -f -- "${SYSTEMD_DIR}/vps-monitor-command.service" \
...
ensure_bin_link() {
    ...
    if ! ln -sfn "$SCRIPT_PATH" "$BIN_PATH"; then
        [[ "$failure_mode" == return ]] && return 1
        fatal "无法创建 ${BIN_PATH} 命令入口。"
    fi
}

assert_fresh_install_targets() {
    local unit
    for unit in vps-monitor-{collect,report,weekly,monthly,auto-update,command}.{service,timer} \
            vps-monitor-boot.service vps-monitor-login.slice; do
...
install_app() {
    ...
    remove_legacy_components
    INSTALL_IN_PROGRESS=0
    success "安装完成，Telegram 状态通知与 SSH 登录提醒已验证"
    printf '\n以后无需操作：每 5 分钟轻量采集，每天中午发送昨天的日报，每周一发送带环比的周报，每月发送月报；每天安全检查 GitHub 更新，VPS 重启恢复和 SSH 登录成功时立即提醒。\n'
...
apply_installed_update() {
    ...
    [[ -e "${SYSTEMD_DIR}/vps-monitor-login.slice" ]] && APPLY_UPDATE_LOGIN_SLICE_EXISTED=1
...
doctor_app() {
    ...
    systemctl reset-failed vps-monitor-{collect,report,weekly,monthly,boot,auto-update}.service \
        vps-monitor-{collect,report,weekly,monthly,auto-update}.timer \
        vps-monitor-login.slice >/dev/null 2>&1 || true
    systemctl enable --now vps-monitor-{collect,report,weekly,monthly,auto-update}.timer >/dev/null
    systemctl enable vps-monitor-boot.service >/dev/null
    remove_legacy_components
    UPDATE_TIMERS_STOPPED=0
...
rename_server() {
    ...
    WORK_DIR="$(mktemp -d -t vps-monitor-rename.XXXXXXXX)"; chmod 0700 "$WORK_DIR"
    trap cleanup_work_dir EXIT
    local new_name temporary
    printf '当前服务器名称：%s\n' "$SERVER_NAME"
    new_name="$(prompt_server_name)"
    [[ "$new_name" != "$SERVER_NAME" ]] || { success "服务器名称没有变化"; return; }
    ...
    if ! printf '%s' "$new_name" > "$temporary" \
        ...
        rm -f -- "$temporary"
        fatal "服务器名称保存失败，原名称未改变。"
    fi
    success "服务器名称已修改为：${new_name}"
    printf '下一次日报、周报、月报、开机提醒和登录提醒将使用新名称。\n'
}
...
uninstall_app() {
    ...
    systemctl stop vps-monitor-login.slice >/dev/null 2>&1 || true
    rm -f -- "${SYSTEMD_DIR}"/vps-monitor-{collect,report,weekly,monthly,auto-update,command}.{service,timer} \
        "${SYSTEMD_DIR}/vps-monitor-boot.service" "${SYSTEMD_DIR}/vps-monitor-login.slice" || true
    if [[ -L "$BIN_PATH" && "$(readlink "$BIN_PATH")" == "$SCRIPT_PATH" ]]; then
        rm -f -- "$BIN_PATH" || true
    ...
    systemctl reset-failed vps-monitor-login.slice >/dev/null 2>&1 || true
    ...
    systemctl is-active --quiet vps-monitor-login.slice && leftovers+=("仍在运行的 systemd 单元 vps-monitor-login.slice")
